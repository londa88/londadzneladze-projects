 ...
 */
bool isTerminal(string cur){
  return cur[0]!='<';
}
void recursion(vector<string> &vect, map<string, Definition> &grammar, string curr){
  if(grammar.find(curr)==grammar.end()){
    vect.push_back(curr);
    return;
  }
  Production curProd=grammar[curr].getRandomProduction();
  for(Production::iterator cur=curProd.begin(); cur!=curProd.end(); cur++){
    string newStr=*cur;
    if(isTerminal(newStr)){
      vect.push_back(newStr);
    }
    else recursion(vect, grammar, *cur);
  }
}
void printRandomParagraph(int vers, vector<string> vect){
...
int main(int argc, char *argv[])
{
 ...
 for(int i=1; i<=3; i++){
  vector<string> vect;
  recursion(vect, grammar, "<start>");
  printRandomParagraph(i, vect);
    }
