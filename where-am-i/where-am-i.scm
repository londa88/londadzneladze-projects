...
(define (quicksort num-list fn)
  (if (<= (length num-list) 1) num-list
      (let ((split (partition (car num-list) (cdr num-list) fn)))
...
(define (intersection-points allTheCircles)
(if (< (length allTheCircles) 2) '()
(append (intersect (car allTheCircles) (cadr allTheCircles))
(intersection-points (cons (car allTheCircles) (cddr allTheCircles)))
(intersection-points (cons (cadr allTheCircles) (cddr allTheCircles))))))


(define (distance-product point allThePoints)
  (apply * (map (lambda (oneOfPoints)
                  (if (equal? point oneOfPoints)
                      1
                      (dist point oneOfPoints)))
                allThePoints)))


(define (rate-points allThePoints)
(map (lambda (point) (list (distance-product point allThePoints) point)) allThePoints))


(define (sort-points allTheRatedPoints)
(quicksort allTheRatedPoints (lambda (ratedP1 ratedP2) (< (car ratedP1) (car ratedP2)))))


(define (clumped-points allThePoints)
(map cadr (prefix-of-list (sort-points (rate-points allThePoints))
(quotient (length allThePoints) 2))))

(define (average-point allThePoints)
(let ((point (list (/ (apply + (map x allThePoints)) (length allThePoints))
(/ (apply + (map y allThePoints)) (length allThePoints)))))
(list (distance-product point allThePoints) point)))


(define (best-estimate allTheCircles)
(average-point (clumped-points (intersection-points allTheCircles))))

(define (where-am-i allTheDistances allTheStaredLocs)
(prefix-of-list (sort-points (map best-estimate (all-guesses allTheDistances allTheStaredLocs))) 6))
